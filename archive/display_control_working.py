  ...
  for control_param,v in control_bounds.items():
    ...
    if type(min_val) == int:
      cv2.createTrackbar(control_param, windowName, \
        controlVal_to_trackbarVal(*controls_bounds[control_param]), \
        # min(max_val, 100), lambda x: on_change(x, control_param))
        min(max_val, 100), partial(on_change, controlParam=control_param))
    ...
    if type(min_val) == float:
      ...
      cv2.createTrackbar(control_param, windowName, \
        ...
        int(max_val), partial(on_change, controlParam=control_param))
